...
public class SaveLoadNewGame {
	...
	public static final String prefix = (System.getProperty("os.name").contains("Windows")?
			// Windows
			(System.getenv("APPDATA")!=null?
				// winxp+
				(System.getenv("APPDATA")+"/jclassicrpg"):
				// other windows
				(System.getProperty("user.home")+"/AppData/Local/jclassicrpg")
			):
			// POSIX/UNIX
			(System.getProperty("user.home")+"/.jclassicrpg"));
	...
	public static void newGame(J3DCore core, Collection<PersistentMemberInstance> partyMembers, CharacterCreationRules cCR, ScenarioDescription desc) 
	{
		...
		try {
			...
			Vector3 closestShrineVector = new Vector3();
			Vector3 tmpVec = new Vector3();
			Vector3 tmpVec2 = new Vector3(wX,wY,wZ);
			double dist = 99999999;
			for (RoadShrine sh:
				world.economyContainer.roadNetwork.shrines)
			{
				tmpVec.set(sh.origoX, sh.origoY, sh.origoZ);
				double dC = tmpVec.distance(tmpVec2);
				if (dist>dC)
				{
					dist = dC;
			...
			wX = (int)closestShrineVector.getX();
			wY = (int)closestShrineVector.getY();
			wZ = (int)closestShrineVector.getZ();
...
/*		if (core.encounterMode) {
		...
		try {
			...
			ScreenExporter.exportCurrentScreen(core.getCanvas().getCanvasRenderer().getRenderer(), _screenShotExp);
			} catch (Exception ex) {}
