...
public class VegetationSetup {
	...
	static BlendState as = new BlendState();
	 ...
	 * @return
	 */
	public static Mesh getVegTrimesh(boolean internal, NodePlaceholder place, RenderedCube c, J3DCore core,TextureStateVegetationModel tm, int k, int j, float heightDiff, float variationCutter)
	{
		...
		if (quads==null) 
		{
			...
			for (int i=0; i<ts.length; i++){
				...
				//if (steepDirection==SurfaceHeightAndType.NOT_STEEP) 
				{
					...
					//Box quad = new Box("grassQuad",new Vector3(),tm.quadSizeX,tm.quadSizeY,tm.quadSizeY);
		    		...
		    		if (tm.atlasTexture)
		    		{
		        		...
		        		FloatBuffer b = quad.getMeshData().getTextureCoords(0).getBuffer();
					...
					if (J3DCore.SETTINGS.DOUBLE_GRASS) {
						Mesh sQ = PooledSharedNode.copyMesh("sharedQuad",quad);
						n.attachChild(sQ);
					}
				} 
				quads[i] = n;
		...
		double quadSeparation = tm.quadSeparation/(J3DCore.SETTINGS.DOUBLE_GRASS?2:1);
		double x = k * quadSeparation + (HashUtil.mixPercentage((int)k, c.cube.x+c.cube.y+c.cube.z+tm.id.length(), (int)j)/variationCutter) - (100/variationCutter/2f);
		double z = j * quadSeparation + (HashUtil.mixPercentage((int)k+1, c.cube.x+c.cube.y+c.cube.z+tm.id.length(), (int)j)/variationCutter) - (100/variationCutter/2f);
		...
		Vector3 translation = new Vector3(x - J3DCore.CUBE_EDGE_SIZE/2, height + tm.quadSizeY/2,-z + J3DCore.CUBE_EDGE_SIZE/2);
		...
		// find rotation
		Vector3 normalY = new Vector3(0,0,1f);//tb.getSurfaceNormal(x, z, null);
		if (normalY == null) {
			normalY = new Vector3(Vector3.UNIT_Y);
		}
		Vector3 normalX = normalY.cross(Vector3.UNIT_X,null);
		Vector3 normalZ = normalY.cross(normalX,null);
		normalX = normalY.cross(normalZ,null);
		Vector3 look = core.getCamera().getDirection().negate(null);
		Vector3 left1 = core.getCamera().getLeft().negate(null);
		Quaternion orient = new Quaternion();
		orient.fromAxes(left1, core.getCamera().getUp(), look);
		//Quaternion rotation = new Quaternion();
		//rotation.fromAxes(normalX, normalY, normalZ);
		Mesh tri = (Mesh)quads[0].getChild(0);
		tri.setRotation(orient);
		tri.setTranslation(translation);
		tri.setScale(scale);
		//if (steepDirection==SurfaceHeightAndType.NOT_STEEP)
		{
	 ...
	 * @return
	 */
	public static Mesh getVegTrimesh(boolean internal, NodePlaceholder place, RenderedCube c, J3DCore core,TextureStateVegetationModel tm, int k, int j, float[] cornerHeights, float variationCutter)
	{
		TextureState[] ts = core.modelLoader.loadTextureStates(tm.textureNames);
		Node[] quads = quadCache.get(tm.getKey()+internal);
		if (quads==null) 
		...
		double zPerc = ((z*1f)/J3DCore.CUBE_EDGE_SIZE);
		double xPerc = 1f-((x*1f)/J3DCore.CUBE_EDGE_SIZE);
	...
	public static Node createVegetation(RenderedCube c, J3DCore core, Camera cam, TextureState[] ts, TextureStateVegetationModel tm) {
		...
		TrimeshGeometryBatch vegetation = new TrimeshGeometryBatch(tm.getKey(),core,(Mesh)quads[0].getChild(0),false,null);
		...
		for (int i = 0; i < quadQuantity; i++) {
			for (int j = 0; j < quadQuantity; j++) {
				double x = i * quadSeparation + (HashUtil.mixPercentage((int)i, c.cube.x+c.cube.y+c.cube.z, (int)j)/150f) - (100/150f/2f);
				double z = j * quadSeparation + (HashUtil.mixPercentage((int)i+1, c.cube.x+c.cube.y+c.cube.z, (int)j)/150f) - (100/150f/2f);
				x = Math.min(x, J3DCore.CUBE_EDGE_SIZE - quadSeparation/4f);
				z = Math.min(z, J3DCore.CUBE_EDGE_SIZE - quadSeparation/4f);
				x = Math.max(x,  + quadSeparation/4f);
				z = Math.max(z,  + quadSeparation/4f);
				...
				// adding CUBE_EDGE_SIZE halfs, and half of the quad to height, to display properly
				Vector3 translation = new Vector3(x - J3DCore.CUBE_EDGE_SIZE/2, -z + J3DCore.CUBE_EDGE_SIZE/2, height + tm.quadSizeY/2);
				...
				rotation.fromAxes(normalX, normalY, normalZ);
				Mesh tri = (Mesh)quads[0].getChild(0);
				tri.setRotation(rotation);
				tri.setTranslation(translation);
				tri.setScale(scale);
				//if (steepDirection==SurfaceHeightAndType.NOT_STEEP)
				{
					rotation.multiplyLocal(new Quaternion(0f, HashUtil.mixPercentage((int)i, c.cube.x+c.cube.y+c.cube.z, (int)j)*3.6f ,0f, 0f));
				}
				/*if (steepDirection!=SurfaceHeightAndType.NOT_STEEP)
				{
