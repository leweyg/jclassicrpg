...
public class GeoTileLoader {
	...
	public class PooledPassNode extends PassNode implements PooledNode
	{
		...
		public void update(NodePlaceholder place) {
			...
			float[][] maps = getHeightMaps(place);
	...
	public PooledNode loadNode(NodePlaceholder nodePlaceholder)
	{
		...
		block.block.getMeshData().copyTextureCoordinates(0, 1, 1);
	...
	public static float HEIGHT_PRECISION_MUL = 1f;
	
	public float[][] getHeightMaps(NodePlaceholder nodePlaceholder)
	{
		...
		for (int i=0; i<4; i++)
		{
			float h = ((cornerHeights[i%4]*HEIGHT_PRECISION_MUL));//*1000f*1000f);
			map[i] = h;
		}
		/*
		...
		*/
			...
			for (int col = 0; col<3; col++)
			{
				...
				} else
				{
					if (row ==0)
					{
						if (opposite!=null)
						{
							bigMap[row*3+col] = (float)((opposite.cube.cornerHeights[1]+oppDeltaY)*HEIGHT_PRECISION_MUL);
						} else
						{
						}
					} else
					if (row ==1)
					{
						if (opposite!=null)
						{
							bigMap[row*3+col] = (float)((opposite.cube.cornerHeights[3]+oppDeltaY)*HEIGHT_PRECISION_MUL);
						}
						else
						{
						}
					...
					if (row == 2)
					{
						if (col==0)
						{
							if (adjacent!=null)
							{
								bigMap[row*3+col] = (float)((adjacent.cube.cornerHeights[2]+adjDeltaY)*HEIGHT_PRECISION_MUL);
							}
							else
							{
							}
						...
						if (col==1)
						{
							if (adjacent!=null)
							{
								bigMap[row*3+col] = (float)((adjacent.cube.cornerHeights[3]+adjDeltaY)*HEIGHT_PRECISION_MUL);
							}
							else
							{
							}
		...
		return new float[][]{map,bigMap};
		
	}
	
	public static double HEIGHT_RATIO = J3DCore.CUBE_EDGE_SIZE/HEIGHT_PRECISION_MUL;
	TiledTerrainBlock b;
	/**
	...
	public TiledTerrainBlockAndPassNode loadNodeOriginal(NodePlaceholder nodePlaceholder, boolean splatNodeNeeded)
	{
		...
		float[][] heightMaps = getHeightMaps(nodePlaceholder);
		...
		{
			block = new TiledTerrainBlockUnbuffered("1",2,new Vector3(J3DCore.CUBE_EDGE_SIZE,HEIGHT_RATIO,J3DCore.CUBE_EDGE_SIZE),heightMaps[0],heightMaps[1],new Vector3(0f,0,0f),false);
			
		} else
		{
			// splatting needs normal buffered block
			block = new TiledTerrainBlock("1",2,new Vector3(J3DCore.CUBE_EDGE_SIZE,HEIGHT_RATIO,J3DCore.CUBE_EDGE_SIZE),heightMaps[0],heightMaps[1],new Vector3(0f,0,0f),false);
		}
		SimpleModel model = (SimpleModel)nodePlaceholder.model;
		//RenderedCube rCube = nodePlaceholder.cube;
		...
		if (ownTexture!=null)
		{
			if (data.getTextureKeyPartForBatch()==null)
			{
	            ...
	            if (model.useAtlasTexture)
	            {
	        		...
	        		FloatBuffer b = block.getMeshData().getTextureCoords(0).getBuffer();
			...
			if (splatNodeNeeded)
			{
		        ...
		        //TextureState ts6 = createLightmapTextureState("../media/test/lightmap.jpg");
    ...
    private TextureState createSplatTextureState(String texture, String alpha) {
		...
		if (state == null)
		{
	    	...
	    	if (t0==null)
	    	{
	        	ResourceSource u = ResourceLocatorTool.locateResource(ResourceLocatorTool.TYPE_TEXTURE, texture);
		        t0 = TextureManager.load(u,
    ...
    private void addAlphaSplat(TextureState ts, String alpha) {
    	...
    	if (t1==null)
    	{
        	ResourceSource u = ResourceLocatorTool.locateResource(ResourceLocatorTool.TYPE_TEXTURE, alpha);
        	if (u!=null)
        	{
    	        t1 = TextureManager.load(u,                
    	        		Texture.MinificationFilter.Trilinear,
    	                false);
    ...
    public static final double getHeight(Vector3 displacementFromCenter, NodePlaceholder place)
    {
    	double x = (displacementFromCenter.getXf() + J3DCore.CUBE_EDGE_SIZE/2f)/J3DCore.CUBE_EDGE_SIZE;
    	double z = -(displacementFromCenter.getZ() - J3DCore.CUBE_EDGE_SIZE/2f)/J3DCore.CUBE_EDGE_SIZE;
    	return getHeight(x, z, place);
    }

    public static final double getHeight(double xPerc, double zPerc, NodePlaceholder place)
    {
    	double NW = place.cube.cube.cornerHeights[0];
		double NE = place.cube.cube.cornerHeights[1];
		double SW = place.cube.cube.cornerHeights[2];
		double SE = place.cube.cube.cornerHeights[3];
