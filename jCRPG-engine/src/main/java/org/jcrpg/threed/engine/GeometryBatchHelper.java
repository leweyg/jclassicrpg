...
public class GeometryBatchHelper {
    ...
    public void addItem(J3DStandingEngine sEngine, boolean internal, Model m, NodePlaceholder place, boolean farView) {
    	...
    	if (J3DCore.ONLY_NEW_TERRAIN && m.type!=Model.TEXTURESTATEVEGETATION && m.type == Model.SIMPLEMODEL && ((SimpleModel)place.model).generatedGroundModel) {
    	...
    	if (m.type!=Model.TEXTURESTATEVEGETATION) {
	    	...
	    	if (batch==null)
	    	{
	    		...
	    		if (J3DCore.SETTINGS.VBO_ENABLED)
	    			batch.getSceneHints().setDataMode(DataMode.VBO);
	    		batch.key = key;
	    		if (m.type == Model.QUADMODEL && ((QuadModel)m).waterQuad)
	    		{
	    			if (J3DCore.waterEffectRenderPass!=null)
	    			{
		    			J3DCore.waterEffectRenderPass.setWaterEffectOnSpatial(batch);
		    			System.out.println("PLACE: "+place.getLocalTranslation());
		    			System.out.println("RPLACE: "+place.getLocalRotation());
			    		if (internal)
			    		{
		    			...
		    			//J3DCore.waterEffectRenderPass.attachChild(batch.parent);
	    			}	    			
	    		} else
	    		{
	    			if (m.transparencyBlendNeeded)
	    			{
		    			batch.parent.setRenderState(core.as);
	    			}
    			...
    			batch.parent.getSceneHints().setCullHint(CullHint.Never); // set culling to NEVER for the first rendering...
    	...
    	if (m.type==Model.TEXTURESTATEVEGETATION) {
	    	...
	    	if (batch==null)
	    	{
	    		Mesh tri = VegetationSetup.getVegTrimesh(internal,place, place.cube, core, (TextureStateVegetationModel)m, 0, 0, 0f, 100f);
    ...
    public void addBillboardVegetationItem(J3DStandingEngine sEngine, boolean internal, Model m, NodePlaceholder place, boolean farView, BillboardPartVegetation vegetationNode) {
    	...
    	if (m.type!=Model.TEXTURESTATEVEGETATION) {
    		...
    		// timeshGeomBatch part (foliage)
    		{
    	    	...
    	    	if (batch==null)
    	    	{
    	    		Mesh tri = vegetationNode.containedFoliageMeshes.iterator().next(); 
    	    	...
    	    	for (Mesh tri:vegetationNode.containedFoliageMeshes)
    	    	{
    	...
    	while (true)
    	{
    		...
    		if (trimeshStepByStepCounter<trimeshBatchMap.size())
    		{
        		...
        		if (batch.parent!=null)
        		{
        			...
        			//batch.lockMeshes(); // XXX you shouldn't lock meshes of Mesh , billboard goes wrong
        			batch.lockShadows();
        			batch.lockTransforms();
        			batch.lockBounds();
        			batch.lockBranch();*/
	    		...
	    		if (batch.visible.size()!=0)
	    		{
	    			if (J3DCore.SETTINGS.SHADOWS)
	    			{
		    			if (batch.model.type==Model.PARTLYBILLBOARDMODEL)
		    			{
		    				...
		    				{
	    						Vector3 pos = batch.parent.getWorldTranslation().add(i.getAttributes().getTranslation(),null);
	    						if (pos.distance(core.getCamera().getLocation())<J3DCore.SETTINGS.RENDER_SHADOW_DISTANCE)
	    						{
	    							core.addOccluder(batch.parent);
	    							found = true;
	    							break;
	    						}
		    				}
		    				if (!found)
		    				{
		    					core.removeOccluder(batch.parent);	
		    				}
		    			}
	    			}
	    			...
	    			if (J3DCore.SETTINGS.SHADOWS)
	    			{
		    			if (batch.model.isShadowNeeded())
		    			{
		    				...
		    				for (ArrayList<GeometryBatchSpatialInstance<GeometryBatchInstanceAttributes>> h:batch.visible.values())
		    				{
		    					...
		    					{
		    						Vector3 pos = batch.getWorldTranslation().add(i.getAttributes().getTranslation(),null);
		    						if (pos.distance(core.getCamera().getLocation())<J3DCore.SETTINGS.RENDER_SHADOW_DISTANCE)
		    						{
		    							core.addOccluder(batch.parent);
		    							found = true;
...
//		    						if (pos.distance(core.getCamera().getLocation())<J3DCore.SETTINGS.RENDER_SHADOW_DISTANCE)
	    		...
	    		if (batch.visible.size()!=0)
	    		{
	    			if (J3DCore.SETTINGS.SHADOWS)
	    			{
		    			if (batch.model.isShadowNeeded())
		    			{
		    				...
		    				for (GeometryBatchSpatialInstance<GeometryBatchInstanceAttributes> i:batch.visible)
		    				{
	    						Vector3 pos = i.getAttributes().getTranslation();
	    						if (pos.distance(core.getCamera().getLocation())<J3DCore.SETTINGS.RENDER_SHADOW_DISTANCE)
	    						{
	    							core.addOccluder(batch.parent);
	    							found = true;
    ...
    {
    	VegetationSetup.quadCache.clear();
    	for (ModelGeometryBatch b:modelBatchMap.values())
    	{
