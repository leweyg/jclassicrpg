...
public abstract class AppBase implements Runnable, Updater, Scene {
    private static final Logger logger = Logger.getLogger(AppBase.class.getName());
    ...
    protected final Node rootNode = _root;
    ...
    protected ScreenShotImageExporter _screenShotExp = new ScreenShotImageExporter(new File(SaveLoadNewGame.shotsDir),"shot","jpg",false);

    protected MouseManager _mouseManager;

    protected JClassicRPGClassicControl _controlHandle;

    protected Vector3 _worldUp = new Vector3(0, 1, 0);

    protected static int _minDepthBits = 8;
    ...
    public void init() {
        ...
        try {
            /*SimpleResourceLocator srl = new SimpleResourceLocator(AppBase.class.getClassLoader().getResource(
                    "com/ardor3d/example/media/"));
            ResourceLocatorTool.addResourceLocator(ResourceLocatorTool.TYPE_TEXTURE, srl);
            srl = new SimpleResourceLocator(AppBase.class.getClassLoader().getResource(
    ...
    public static void start(AppBase example) {
        // Ask for properties
        ...
        prefs.setDepthBits(8);
        prefs.setStencilBits(0);
        prefs.setAlphaBits(0);
        
        //prefs.setAlphaBits(8);
        //prefs.setStencilBits(8);
    ...
    public static void start2(Class<? extends AppBase> exampleClazz) {
        // Ask for properties
        final PropertiesGameSettings prefs = getAttributes(new PropertiesGameSettings("ardorSettings.properties", null));
    ...
    protected static PropertiesGameSettings getAttributes(final PropertiesGameSettings settings) {
        ...
        if (dflt != null) {
            try {
                dialogImage = AppBase.class.getResource(dflt);
        ...
        try {
            ...
            logger.logp(Level.SEVERE, AppBase.class.getClass().toString(), "ExampleBase.getAttributes(settings)",
                    "Exception", e);
            return null;
        }
    ...
    protected void registerInputTriggers() {
        ...
        if (this instanceof J3DCore)
        {
        	_controlHandle = JClassicRPGClassicControl.setupTriggers(_logicalLayer, _worldUp, false);
        }
        ...
        if (J3DCore.SETTINGS.QUICK_EXIT)
        {
