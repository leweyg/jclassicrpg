...
import com.ardor3d.extension.terrain.ClipmapLevel;
...
public class TexturedGeometryClipmapTerrain extends Node {
    ...
    public ReadOnlyVector3 lastUpdateLocation = null;
    public ReadOnlyVector3 lastUpdateDirection = null;
    ...
    protected void updateChildren(final double time) {
        ...
        final ReadOnlyVector3 loc = _terrainCamera.getLocation();
        final ReadOnlyVector3 dir = _terrainCamera.getDirection();

        if (lastUpdateLocation != null) {
            if (loc.distance(lastUpdateLocation) < 0.1f) {
                if (dir.distance(lastUpdateDirection) < 0.05f) {
                    return;
                }
            }
        }
        lastUpdateLocation = new Vector3(loc);
        lastUpdateDirection = new Vector3(dir);
    ...
    public void reloadShader() {
        ...
        if (caps.isGLSLSupported()) {
            ...
            _geometryClipmapShader.setUniform("minDist", minDist);
