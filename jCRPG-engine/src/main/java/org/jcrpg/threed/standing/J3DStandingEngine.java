...
public class J3DStandingEngine {
	...
	private void renderNodes(NodePlaceholder[] n, RenderedCube cube, int relX, int relY, int relZ, int x, int y, int z, int direction, int horizontalRotation, float scale)
	{
		...
		double cX = ((x+relX)*J3DCore.CUBE_EDGE_SIZE+1f*((int[])f[1])[0]*(cube.farview?J3DCore.FARVIEW_GAP:1));//+0.5f;
		double cY = ((y+relY)*J3DCore.CUBE_EDGE_SIZE+1f*((int[])f[1])[1]*(cube.farview?J3DCore.FARVIEW_GAP:1));//+0.5f;
		double cZ = ((z-relZ)*J3DCore.CUBE_EDGE_SIZE+1f*((int[])f[1])[2]*(cube.farview?J3DCore.FARVIEW_GAP:1));//+25.5f;
		if (cube.farview)
		{
		...
		if (currentNode<alCurrentCubes.size()) 
		{
			...
			while (true)
			{
				...
				if (c.hsRenderedNodes.size()>0)
				{
					...
					int distX = Math.abs(core.gameState.getCurrentRenderPositions().viewPositionX+1-c.cube.x); // +1 so new a3d terrain edge wont have gap
					int distY = Math.abs(core.gameState.getCurrentRenderPositions().viewPositionY-c.cube.y);
					int distZ = Math.abs(core.gameState.getCurrentRenderPositions().viewPositionZ-1-c.cube.z); // -1 so new a3d terrain edge wont have gap
								...
								if (overrideBatch)
								{
									...
									else
									{
										...
										}*/
										{
											...
											/* ardor3d if (realPooledNode instanceof BillboardPartVegetation)
											{
	...
	public void updateAfterRender()
	{
		...
		if (newNodesToSetCullingDynamic.size()>0)
		{
			...
			while (true)
			{
				...
				if (n.getChildren()!=null && n.getChildren().size()==1)
				{
					...
					{
						//n.unlock();
						/*if (s instanceof ModelGeometryBatch)
						{
							core.toCompile.add(s);
						}*/
		...
		// before lockAll (committing batch mesh buffers), we need to set commit to true
