...
public class WorldMap {
	...
	public void paintPattern(byte[] map, int X, int Y, int sizeX, ColorRGBA[][] pattern, boolean zoomed)
	{
		...
		for (int x = 0; x<PIXELS_PER_BLOCK; x++)
		{
			for (int z = 0; z<PIXELS_PER_BLOCK; z++)
			{
				...
				if (pattern[patternY][patternX]!=TR_COL || pattern[patternY][patternX].getAlpha()>0f) {
					int disp = 4 * (X*PIXELS_PER_BLOCK + x + (Y * PIXELS_PER_BLOCK * PIXELS_PER_BLOCK + z * PIXELS_PER_BLOCK)* sizeX);
					for (int i=0; i<4; i++)
					{
							map[disp+i] = (byte)((int)((pattern[patternY][patternX].toArray(null)[i]*255f)));
					}
				}
			}
		}
	...
	BufferedImage posBuffImg;
	
	public WorldMap(World w) {
		world = w;
		
		posBuffImg = new BufferedImage(w.sizeZ* PIXELS_PER_BLOCK, w.sizeX*PIXELS_PER_BLOCK, BufferedImage.TYPE_4BYTE_ABGR);
		final byte data[] = AWTImageLoader.asByteArray(posBuffImg);
		posTexState = new TextureState();
		Texture2D tx = new Texture2D();
		ByteBuffer buff = BufferUtils.createByteBuffer(data.length);
		Image img = new Image(ImageDataFormat.RGBA, ImageDataType.Byte,  w.sizeZ* PIXELS_PER_BLOCK, w.sizeX*PIXELS_PER_BLOCK, buff, null);
		tx.setImage(img);
		TextureKey rtt = TextureKey.getRTTKey(MinificationFilter.NearestNeighborNoMipMaps);
		tx.setTextureKey(rtt);
		posTexState.setTexture(tx);
		
		positionGraphics = posBuffImg.getGraphics();
		...
		for (int z = 0; z<w.sizeZ;z++)
		{
			for (int x=0; x<w.sizeX; x++)
			{
				...
				mapImage[((z*w.sizeX*PIXELS_PER_BLOCK*PIXELS_PER_BLOCK)+x*PIXELS_PER_BLOCK)*4+3] = (byte)200;
		...
		oceanImage.setDataFormat(ImageDataFormat.RGBA);
		oceanImage.setHeight(w.sizeZ*PIXELS_PER_BLOCK);
		oceanImage.setWidth(w.sizeX*PIXELS_PER_BLOCK);
		ByteBuffer buffer = BufferUtils.createByteBuffer(mapImage.length);//.wrap(mapImage);
		for (byte b:mapImage)
			buffer.put(b);
		oceanImage.setData(buffer);
		
		positionImage = new Image();
		positionImage.setDataFormat(ImageDataFormat.RGBA);
		positionImage.setHeight(w.sizeZ*PIXELS_PER_BLOCK);
		positionImage.setWidth(w.sizeX*PIXELS_PER_BLOCK);
		ByteBuffer buffer2 = BufferUtils.createByteBuffer(positionImageSet.length);//ByteBuffer.wrap(positionImageSet);
		positionImage.setData(buffer2);
		
		geoImage = new Image();
		geoImage.setDataFormat(ImageDataFormat.RGBA);
		geoImage.setHeight(w.sizeZ*PIXELS_PER_BLOCK);
		geoImage.setWidth(w.sizeX*PIXELS_PER_BLOCK);
		ByteBuffer buffer3 = BufferUtils.createByteBuffer(geoImageSet.length);//ByteBuffer.wrap(geoImageSet);
		for (byte b:geoImageSet)
			buffer3.put(b);
	...
	public void paintPoint(Graphics set, int x, int y, int r, int g, int b, int a)
	{
		a = 255;
		r = 255;
	...
	public void update(int cx, int cy, int cz)
	{
		...
		if (cleanArray==null)
		{
			cleanArray = new int[posBuffImg.getWidth()*posBuffImg.getHeight()*4];
		}
		posBuffImg.setRGB(0, 0, posBuffImg.getWidth(), posBuffImg.getHeight(), cleanArray , 0, 0);
		...
		{
			if (q.getLocalRenderState(StateType.Texture)!=posTexState) {
				q.setRenderState(posTexState);
			}
		}
		updated = true;
	}
	public Graphics positionGraphics;

	
    private ByteBuffer imageBuffer;

    private double throttle = 1d/2d;
    ...
    public boolean wasShownAtLeastOnce = false;
	public void render(final Renderer renderer) {
		
//		if (!updated) return;
		if (!wasShownAtLeastOnce) return;
		...
		if (sinceLast>throttle)
		{
	        ...
	        imageBuffer.flip();
	        final Texture prevTexture = ((TextureState) posTexState)//.getLocalRenderState(RenderState.StateType.Texture))
	                .getTexture();
	        renderer.updateTexture2DSubImage((Texture2D) prevTexture, 0, 0, posBuffImg.getWidth(), posBuffImg.getHeight(),
	                imageBuffer, 0, 0, posBuffImg.getWidth());
	...
	public TextureState[] getMapTextures()
	{
		...
		oceanTex.setTextureKey(TextureKey.getRTTKey(MinificationFilter.NearestNeighborNoMipMaps));
		...
		geoTex.setTextureKey(TextureKey.getRTTKey(MinificationFilter.NearestNeighborNoMipMaps));
		...
		baseTexState = new TextureState();
		baseTexState.setTexture(oceanTex);
		geoTexState = new TextureState();
