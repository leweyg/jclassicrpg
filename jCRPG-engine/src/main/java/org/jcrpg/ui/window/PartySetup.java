...
public class PartySetup extends PagedInputWindow {
	...
	public PartySetup(UIBase base) {
		...
		try {
			...
			// page selection -----------------------------------------------
			Quad hudQuad = loadImageToQuad("/baseWindowFrame.dds", 0.8f*core.getDisplay().getWidth(), 1.61f*(core.getDisplay().getHeight() / 2), 
	    			core.getDisplay().getWidth() / 2, 1.13f*core.getDisplay().getHeight() / 2);
	    	hudQuad.setRenderState(base.hud.hudAS);
	    	pageMemberSelection.attachChildAt(hudQuad,0);
	    	...
	    	pageCreationFirst.attachChildAt(sQuad,0);
	    	...
	    	Mesh sQuad2 = PooledSharedNode.copyMesh("--",hudQuad);
	    	sQuad2.setTranslation(hudQuad.getTranslation());
	    	pageCreationSecond.attachChildAt(sQuad2,0);
			...
			pageMemberSelection.updateWorldBound(true);
		} catch (Exception ex)
		{
			ex.printStackTrace();
		}
	...
	public void setupPage()
	{
		if (currentPage==0 && !noNeedForRefreshPage)
		{
			...
			windowNode.attachChildAt(pageMemberSelection,0);
		...
		{
			windowNode.detachAllChildren();
			windowNode.attachChildAt(pageCreationFirst,0);
		...
		{
			windowNode.detachAllChildren();
			windowNode.attachChildAt(pageCreationSecond,0);
	...
	{
		refreshCharacterList(SaveLoadNewGame.charsDir);
	}
	
	public void refreshCharacterList(String dir)
	{
		try {
	...
	public boolean inputLeft(InputBase base, String message) {
		...
		if (base.equals(genderSelect))
		{
			...
			String path = "../media/portraits/"+race.pictureRoot+"/"+genderPath;
	...
	private AudioDescription lastDesc = null;
	
	
	public void playVoiceType(AudioDescription desc)
	{
		if (lastDesc!=null && lastDesc == desc) return;
		lastDesc = desc;
	...
	public boolean inputChanged(InputBase base, String message) {
		...
		if (base.equals(addCharSelect))
		{
			...
			try {
				...
				if (q==null) 
				{
					q = loadImageToQuadDirectly(d.pic,  0.12f*core.getDisplay().getWidth(), (0.16f)*core.getDisplay().getHeight(),0.75f*core.getDisplay().getWidth(), (1f-0.2f)*core.getDisplay().getHeight()  );
					imgQuads.put(d.pic.getAbsolutePath(), q);
				}
				imageNode.detachAllChildren();
				imageNode.attachChildAt(q,0);
				//imageNode.attachChildAt(imageNode,0);
				pageMemberSelection.attachChildAt(imageNode,0);
			} catch (Exception ex)
			{
