...
public class StorageHandlingWindow extends PagedInputWindow {
	...
	public StorageHandlingWindow(UIBase base) {
		super(base);
		try {
			Quad hudQuad = loadImageToQuad("/nonPatternFrame1.png", 0.75f*core.getDisplay().getWidth(), 0.75f*(core.getDisplay().getHeight() / 2), 
	...
	private void updateInventoryList()
	{
		...
		for (InventoryListElement invListElement:listOfAll)
		{
			...
			try {
				icons[count] = UIImageCache.getImage(UIBase.RES_TYPE_GAME_2D,invListElement.description.getIconFilePath(), true,15f,15f);
			} catch (Exception ex)
			{
				ex.printStackTrace();
			}
