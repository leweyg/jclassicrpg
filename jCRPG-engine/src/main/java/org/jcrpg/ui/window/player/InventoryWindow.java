...
public class InventoryWindow extends PagedInputWindow {
	...
	public InventoryWindow(UIBase base) {
		super(base);
		try {
			Quad hudQuad = loadImageToQuad("/nonPatternFrame1.png", 0.75f*core.getDisplay().getWidth(), 1.7f*(core.getDisplay().getHeight() / 2), 
	    	...
	    	sQuad = PooledSharedNode.copyMesh("",hudQuad);
	    	sQuad.setTranslation(hudQuad.getTranslation());
	    	page1_details.attachChildAt(sQuad,0);
	...
	{
		{
			...
			for (InventoryListElement objEl:list) {
				...
				try {
					icons[counter] = UIImageCache.getImage(UIBase.RES_TYPE_GAME_2D, objEl.description.getIconFilePath(), true,15f,15f);
				} catch (Exception ex)
				{
					ex.printStackTrace();
				}
	...
	private void updateDetailPageToInvListElement(InventoryListElement element)
	{
		...
		try {
			currentDetailQuad = loadImageToQuad(UIBase.RES_TYPE_GAME_2D, element.description.getIconFilePath(), 0.17f*core.getDisplay().getWidth(), 0.20f*(core.getDisplay().getHeight()),
				core.getDisplay().getWidth() / 2, 1.45f*core.getDisplay().getHeight() / 2);
			page1_details.attachChildAt(currentDetailQuad,0);
		} catch (Exception ex)
		{
	...
	public void hide() {
		...
		if (fallbackWindow!=null) {
			...
			core.getInputHandler().setForbidWindowEventHandling(true);
			fallbackWindow = null;
			canDoActions = true;
		}
	}
