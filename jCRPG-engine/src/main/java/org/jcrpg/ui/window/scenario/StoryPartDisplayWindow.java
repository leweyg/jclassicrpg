...
public class StoryPartDisplayWindow extends InputWindow implements Runnable{
	...
	public ArrayList<org.jcrpg.ui.BasicText> scrollingLines = new ArrayList<org.jcrpg.ui.BasicText>();
	...
	{
		private double fliedTime= 0f;
		private double maxTime = 1.5f;
		private boolean flying = false;
		private double speed = 0.1f;
		
		public ArrayList<Runnable> onFinish = new ArrayList<Runnable>();
		
		public ArrayList<org.jcrpg.ui.BasicText> texts = new ArrayList<org.jcrpg.ui.BasicText>();
		
		@Override
		public void updateGeometricState(double time, boolean initiator) {
			if (flying && fliedTime<maxTime) {
				setTranslation(getTranslation().add(0d, speed*time*(core.getDisplay().getHeight()/600d) / ((fliedTime+maxTime/1.7d)/maxTime), 0d,null));
				fliedTime+=1f*time;
				for (org.jcrpg.ui.BasicText t:texts)
				{
					double y = Math.min(1d,(t.getWorldTranslation().getY()/core.getDisplay().getHeight())*8d) ;
					/*t.getTextColor().b = y;
					t.getTextColor().r = y;
					t.getTextColor().g = y;
					t.getTextColor().a = y; TODO ardor3d text*/
				}
			} else
			if (flying && fliedTime>=maxTime)
			{
	...
	public StoryPartDisplayWindow(UIBase base) {
		super(base);
		try {
			Quad q = loadImageToQuad("dark_mask.png", core.getDisplay().getWidth(), core.getDisplay().getHeight(), 
		...
		{
			org.jcrpg.ui.BasicText t = org.jcrpg.ui.BasicText.createDefaultTextLabel("demo_line_"+i,"");
			t.setTranslation(0.15f * width, height - i* height*0.05f, 0f);
			t.setScale(width/1330f);
			scrollingLines.add(t);
			scrollNode.attachChildAt(t,0);
			scrollNode.texts.add(t);
		}
		windowNode.attachChildAt(scrollNode,0);
	...
	private void playBlock(int block)
	{
		...
		for (String line:lines)
		{
			...
			{
				scrollingLines.get(counter++).setText(line);
			}
		}
		if (counter<scrollingLines.size()-1)
		{
			...
			{
				scrollingLines.get(i).setText("");
			}
		}
		scrollNode.setTranslation(0f, 0-core.getDisplay().getHeight(), 0f);
