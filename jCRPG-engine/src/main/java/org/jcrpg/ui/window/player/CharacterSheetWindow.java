...
public class CharacterSheetWindow extends PagedInputWindow {
	...
	private void updateToMemberInstance(EntityMemberInstance instance)
	{
		...
		try {
			...
			currentPic.setTranslation(1.6f*core.getDisplay().getWidth()/2,1.55f*core.getDisplay().getHeight()/2,0);
			currentPic.setScale(1f* (core.getDisplay().getWidth()/640f));
			page0.attachChildAt(currentPic,0);
		} catch (Exception ex)
		{
			ex.printStackTrace();
		}
	...
	public void hide() {
		...
		if (fallbackWindow!=null) 
		{
			...
			core.getInputHandler().setForbidWindowEventHandling(noToggleWindowByKeySettingAfterFallbackWindowUse);
			fallbackWindow = null;
		}
	}
	@Override
