...
public class ListMultiSelect extends InputBase {
	...
	org.jcrpg.ui.BasicText dText, dFlag;
	...
	public void setupDeactivated()
	{
		...
		if (maxCount==0 || selected+fromCount>=texts.length) {
		...
		if (J3DCore.NATIVE_FONT_RENDER)
		{
			...
			org.jcrpg.ui.BasicText t = dText;
		...
		if (flag!=null) 
		{
			...
			if (J3DCore.NATIVE_FONT_RENDER)
			{
				...
				org.jcrpg.ui.BasicText t = dFlag;
				signNode = new Node();
				signNode.attachChildAt(t,0);
				float scale = w.core.getDisplay().getWidth()/fontRatio/TEXT_PROP;
				t.setScale(scale);
				signNode.setTranslation(t.getCenterOrigoX(dCenterSignX,scale), - t.getHeight2()/2f*scale + dCenterY,0);
				signNode.getSceneHints().setRenderBucketType(RenderBucketType.Ortho);
				t.setTextColor(new ColorRGBA(1,1,0.1f,1f));
			}
			baseNode.attachChildAt(signNode,0);
		}
	...
	public org.jcrpg.ui.BasicText[] flagInstances = new org.jcrpg.ui.BasicText[maxVisible+1];
	...
	public void setupActivated()
	{
		...
		for (int i=0; i<maxVisible+1; i++) {
			if (i+fromCount<maxCount) {
				...
				if (flag!=null) 
				{
					...
					if (J3DCore.NATIVE_FONT_RENDER)
					{
						org.jcrpg.ui.BasicText t = flagInstances[i];
						if (t==null) 
						{
							t = createText(flag);
							flagInstances[i] = t;
						...
						float scale = w.core.getDisplay().getWidth()/fontRatio/TEXT_PROP;
						t.setScale(scale);
						signNode.setTranslation(t.getCenterOrigoX(dCenterSignX,scale), - t.getHeight2()/2f*scale + dCenterY - dSizeY*i,0);
						signNode.getSceneHints().setRenderBucketType(RenderBucketType.Ortho);
						t.setTextColor(new ColorRGBA(1,1,0.1f,1f));
					}
					baseNode.attachChildAt(signNode,0);
		...
		activatedNode.getChild(0).setScale(new Vector3(1f,size,size));
		if (size>0)
		{
			activatedNode.getChild(0).setTranslation(dCenterX, dCenterY - ((size-1) * dSizeY)/2, 0);
		}
	}
