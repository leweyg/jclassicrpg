...
public class TurnActWindow extends PagedInputWindow {
	...
	public TurnActWindow (UIBase base) {
		...
		try {
			Quad hudQuad = loadImageToQuad("/popups/turnact.png", 0.6f*core.getDisplay().getWidth(), 1.2f*(core.getDisplay().getHeight() / 2), 
		...
		try {
			Quad hudQuad = loadImageToQuad("/nonPatternFrame1_trans.png", 0.75f*core.getDisplay().getWidth(), 1.67f*(core.getDisplay().getHeight() / 2), 
			...
			Quad shieldQuad = loadImageToQuad("/windowicons/turnact_shield.png", 0.1f*core.getDisplay().getWidth(), 0.1f*(core.getDisplay().getHeight()), 
