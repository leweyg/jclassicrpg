...
public class AudioServer implements Runnable {
	...
	public static final String PATH_TO_MEDIA = "../media/";
	...
	public AudioServer()
	{
		...
		try {
			AudioTrack mainTheme = AudioSystem.getSystem().createAudioTrack(new File(PATH_TO_MEDIA+"audio/music/fantasy/fantasy_menu.ogg").toURL(), true);
		...
		try {
			AudioTrack mainTheme = AudioSystem.getSystem().createAudioTrack(new File(PATH_TO_MEDIA+"audio/music/struggle/combat.ogg").toURL(), true);
			mainTheme.setType(TrackType.MUSIC);
			mainTheme.setRelative(false);
			mainTheme.setLooping(true);
			mainTheme.setVolume(J3DCore.SETTINGS.MUSIC_VOLUME_PERCENT/100f);
		...
		try {
			AudioTrack mainTheme = AudioSystem.getSystem().createAudioTrack(new File(PATH_TO_MEDIA+"audio/music/stones/tense.ogg").toURL(), true);
			mainTheme.setType(TrackType.MUSIC);
			mainTheme.setRelative(false);
			mainTheme.setLooping(true);
			mainTheme.setVolume(J3DCore.SETTINGS.MUSIC_VOLUME_PERCENT/100f);
		...
		try {
			AudioTrack mainTheme = AudioSystem.getSystem().createAudioTrack(new File(PATH_TO_MEDIA+"audio/music/horizon/leveling.ogg").toURL(), true);
			mainTheme.setType(TrackType.MUSIC);
			mainTheme.setRelative(false);
			mainTheme.setLooping(true);
			mainTheme.setVolume(J3DCore.SETTINGS.MUSIC_VOLUME_PERCENT/100f);
		...
		{
			try {
				AudioTrack mainTheme = AudioSystem.getSystem().createAudioTrack(new File(PATH_TO_MEDIA+"audio/sound/steps/"+step+".ogg").toURL(), false);
				...
				ArrayList<AudioTrack> tracks = new ArrayList<AudioTrack>();
				tracks.add(mainTheme);
				hmTracksAndFiles.put(step, PATH_TO_MEDIA+"audio/sound/steps/"+step+".ogg");
		...
		addTrack(EVENT_ENC1, PATH_TO_MEDIA+"audio/sound/events/enc1.ogg");
	...
	public boolean setSoundPlayingVolume(String id,double volume)
	{
		boolean b = false;
		for (Channel c:channels)
		{
			...
			if (!c.isAvailable() && c.soundId.equals(id))
			{
				...
				c.track.setTargetVolume((float)(volume* J3DCore.SETTINGS.EFFECT_VOLUME_PERCENT/100f));
	...
	public synchronized AudioTrack getPlayableTrack(String id, boolean continuous) {
		...
		} catch (Exception ex)
		{
			if (J3DCore.SETTINGS.SOUND_ENABLED) {
				if (Jcrpg.LOGGER.getLevel()!= Level.OFF) {
					Jcrpg.LOGGER.finest("AudioServer getPT: "+ex.toString());
				}
				//ex.printStackTrace();
			}
			return null;
	...
	public synchronized void playLoading(String id, String type)
	{
		...
		try {
			...
			if (track==null) {
				...
				track = addTrack(id, PATH_TO_MEDIA+"audio/sound/"+type+"/"+id+".ogg");
			}
			if (!track.getType().equals(TrackType.MUSIC)) {
				track.setVolume(J3DCore.SETTINGS.EFFECT_VOLUME_PERCENT/100f);
			}
	...
	public synchronized void playLoadingRandomPitch(String id, String type, int maxDeltaPercentage)
	{
		...
		try {
			...
			{
				track.setPitch(1.0f+ (random.nextInt(maxDeltaPercentage)-(maxDeltaPercentage/2))/100f);
			} else
			{
				track.setPitch(1.0f);
			}
	...
	public synchronized void playContinuousLoading(String id, String type, double volume)
	{
		if (!J3DCore.SETTINGS.SOUND_ENABLED) return;
		if (id==null) return;
		...
		try {
			...
			if (tracks==null)
			{
				track = addTrackStreaming(id, PATH_TO_MEDIA+"audio/sound/"+type+"/"+id+".ogg");
			...
			if (track.isPlaying()) {
				track.setTargetVolume((float)(volume * J3DCore.SETTINGS.EFFECT_VOLUME_PERCENT/100f));
				return;
			} else
			{
				c.setTrack(id, track);
	...
	public void playSoundForRandomMember(PartyInstance party, String type)
	{
		...
		{
			outCount++;
			int m = random.nextInt(party.fixMembers.size());
			int counter = 0;
			String sound = null;
			float pitch = 1.0f;
