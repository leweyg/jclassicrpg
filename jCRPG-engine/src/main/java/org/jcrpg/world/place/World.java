...
import org.jcrpg.game.GameStateContainer.ScenarioPositions;
...
import org.jcrpg.threed.core.texture.TexturizerWithBlendArrayFunction3D;
...
public class World extends Place implements TileBasedMap, Heightmap, TexturizerWithBlendArrayFunction3D {
	...
	int count = 0;
	int countCache = 0;
	ArrayList<SurfaceHeightAndType[]> tempHeightData = new ArrayList<SurfaceHeightAndType[]>();
	class CommonTerrainCacheData
	{
		public Cube c;
		public Float h;
	}
	transient HashMap<Long, CommonTerrainCacheData> cache = new HashMap<Long, CommonTerrainCacheData>();
	public float getHeight(int x, int z) {
		
		if (!Jcrpg.TOOL_SESSION)
		{
			ScenarioPositions pos = J3DCore.getInstance().gameState.getNormalPositions();
			x = getX(x,pos);
			z = getZ(z,pos);
		}
		long k = WorldSizeBitBoundaries.getKey(x, 0, z);
		if (cache==null) cache = new HashMap<Long, CommonTerrainCacheData>();
		CommonTerrainCacheData r = cache.get(k);
		if (r!=null) 
		{
			countCache++;
			if (countCache%1000==0)
			{
				System.out.print("COUNT CACHE $ "+countCache);
			}
			return r.h;
		}
		count++;
		if (count%1000==0)
		{
			System.out.print("COUNT "+count+ " "+cache.size());
		}
		{
			getSurfaceData(x, z, tempHeightData);
			for (SurfaceHeightAndType[] d : tempHeightData) {
				for (SurfaceHeightAndType t : d) {
					if (t.self.returnsGeoOutsideHeight == false)
						continue;
					int h = t.surfaceY;
					Cube c = getCube(tt, -1, x, h, z, false);
					float middleHeight = 0;
					if (c != null) {
						middleHeight = c.middleHeight;
					}
					Float h1 = (((h + middleHeight - 40.5f) * 2f)) / 1000f;
					CommonTerrainCacheData cd = new CommonTerrainCacheData();
					cd.h = h1;
					cd.c = c;
					synchronized (cache){
						if (cache.size()>30000)
						{
							System.out.println("%%%%%%%%%%%%%%%%%%%%%%%%%%% Height CACHE CLEAR %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
							cache.clear();
						}}
					cache.put(k, cd);
					return h1;
		...
		CommonTerrainCacheData cd = new CommonTerrainCacheData();
		Float h1 = getPointHeightOutside(x, z, false);
		h1 = h1*2f/1000f;
		cd.h = h1;
		synchronized (cache){
		if (cache.size()>30000)
	...
	private transient HashMap<Long, double[]> evalCache = new HashMap<Long, double[]>();

	
	private int getX(int X,ScenarioPositions pos)
	{
		
		int x = shrinkToWorld((TerrainHelper.startingX+pos.origoX+X));
		return x;
	}
	private int getZ(int Z,ScenarioPositions pos)
	{
		
		int z = shrinkToWorld((-TerrainHelper.startingZ+pos.origoZ-Z));
		return z;
	}
	
	public double[] eval(double dx, double dy, double dz) {
		
		int iX, iZ;
		iX = (int)(dx*1.25d/(J3DCore.CUBE_EDGE_SIZE*J3DCore.CUBE_EDGE_SIZE));///(100d/(TerrainHelper.TEXTURE_SCALE*TerrainHelper.TEXTURE_SCALE)));
		iZ = (int)(dy*1.25d/(J3DCore.CUBE_EDGE_SIZE*J3DCore.CUBE_EDGE_SIZE));///(100d/(TerrainHelper.TEXTURE_SCALE*TerrainHelper.TEXTURE_SCALE)));// + 16;
		...
		if (x==pos.viewPositionX && z==pos.viewPositionZ)
		{
			//System.exit(-1);
		}
		...
		} else
		{
			...
			ArrayList<SurfaceHeightAndType[]> data = getSurfaceData(x, z, null);
			for (SurfaceHeightAndType[] d:data)
			{
				for (SurfaceHeightAndType t:d)
				{
					if (t.self.returnsGeoOutsideHeight==false) continue;
					int h = t.surfaceY;
					c= getCube(tt, -1, x, h, z, false);
					float middleHeight = 0;
					if (c != null) {
						middleHeight = c.middleHeight;
					}
		...
		if (c!=null && Continental.CONTINENTAL_ID.equals(c.climateId))
		{
			return new double[] { 1d, 0d };
		} else
		if (c != null && Desert.DESERT_ID.equals(c.climateId)) 
		{
			return new double[] { 0d, 0d, 0d, 1d, 0d};
		} else 
		if (c != null && Tropical.TROPICAL_ID.equals(c.climateId)) 
		{
			return new double[] { 0d, 0d, 0d, 0d, 1d };
		} else 
		if (c != null && Arctic.ARCTIC_ID.equals(c.climateId)) 
		{
			return new double[] { 0d, 0d, 1d, 0d, 0d };
		} else 
		{
			return new double[] { 0d, 1d, 0d, 0d, 0d };
		}

	}

	
	final public String[] orderedTerrainImages = new String[]{"grass2.png","twater.png","snow1.png","sand2.png","jungle.png"};

	public BufferedImage[] getImages() {
		BufferedImage[] images = new BufferedImage[orderedTerrainImages.length];
		int i =0;
		for (String s:orderedTerrainImages)
		{
			try {
				System.out.println(s);
				BufferedImage img = ImageIO.read(new File("../media/textures/models/low_png/"+s));
				images[i++] = img; 
			}
			catch (Exception ex)
			{
