...
public class EntityFragments {
	...
	public class EntityFragment implements EncounterUnit 
	{
		...
		public ArrayList<PerceptVisibleForm> getPerceptedForms(PerceptEntityData perceptedData) {
			...
			for (int i=0; i<form2.getSize();i++)
			{
				...
				form.uniqueId=i;
				form.worldX = (int)getRoamingPosition().getX();
				form.worldY = (int)getRoamingPosition().getY();
				form.worldZ = (int)getRoamingPosition().getZ();
		...
		public ArrayList<EncounterUnit> getSubUnits(int posX, int posY, int posZ) {
			...
			for (PersistentMemberInstance pi:followingMembers) {
				if (tmpVector==null) tmpVector = new Vector3();
		...
		{
			return new Vector3(roamingBoundary.posX,roamingBoundary.posY,roamingBoundary.posZ);
		}
	...
	public transient Vector3 tmpVector = new Vector3();
