				...
				for (int i=0; i<groupSize; i++) {
					if (i<livingMembers.size()) 
					{
						...
						if (u!=null)
						{
							...
							if (inst.isDead())
							{
								unit.applyMessages.add(new TextEntry(""+inst.description.getName()+" dies!",TextEntry.RED));
								deadMembers.add(inst);
								killCount++;
							}
							if (inst.memberState.isNeutralized())
							{
								unit.applyMessages.add(new TextEntry(""+inst.description.getName()+" neutralized!",TextEntry.ORANGE));
		...
		} else
		{
			if (subUnit instanceof EntityMemberInstance)
			{
				...
				if (u!=null)
				{
					...
					{
						unit.applyMessages.add(new TextEntry(""+subUnit.getName()+" dies!",TextEntry.RED));
						killCount++;
					...
					{
						unit.applyMessages.add(new TextEntry(""+subUnit.getName()+" neutralized!",TextEntry.ORANGE));
						neutralizeCount++;
