...
public class PerceptEntityData {
	...
	public Double getUpdatedDist()
	{
		if (distanceKnown)
		{
			Vector3 point = unit.getFragment().getRoamingPosition();//new Vector3f(commonRadius[1][0],commonRadius[1][1],commonRadius[1][2]);
			distance =  source.getRoamingPosition().distance(point);
		}
		return distance;
	}
	
	public void updateToResultRatio(float result, float resultIdentification, Vector3 sourcePos, EncounterUnit target)
	{
		if (result > 0.1f)
		{
			percepted = true;
		...
		if (distanceKnown)
		{
			...
			Vector3 point = target.getFragment().getRoamingPosition();//new Vector3f(commonRadius[1][0],commonRadius[1][1],commonRadius[1][2]);
