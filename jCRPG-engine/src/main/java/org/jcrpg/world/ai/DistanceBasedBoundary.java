...
public class DistanceBasedBoundary extends Boundaries {
	...
	public boolean isInside(int absoluteX, int absoluteY, int absoluteZ) {
		...
		//int y = absoluteY;
		int z = absoluteZ;
		Vector3 v = VectorPool.getVector3(x, 0, z);
		double dist = v.distance(pv);
		VectorPool.releaseVector3(v);
	...
	public static int[][] getCommonRadiusRatiosAndMiddlePoint(DistanceBasedBoundary one, DistanceBasedBoundary two)
	{
		...
		//  |        .    |    .  |  |
		if (true) {
			Vector3 normalizedDistDirVect = bigger.pv.subtract(smaller.pv,null).normalize(null);
			float f = (common_distance * 1f ) / 2f;
			Vector3 middle = smaller.pv.add(normalizedDistDirVect.multiply(smaller.getRadiusInRealCubes(),null),null).subtract( normalizedDistDirVect.multiply(f,null), null );
			return new int[][]{ {common_distance*50 /  one.getRadiusInRealCubes(), common_distance*50 /  two.getRadiusInRealCubes()}, {(int)middle.getX(), two.posY+((one.posY-two.posY)/2), (int)middle.getZ()}};
		}
