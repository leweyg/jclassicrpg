...
import org.jcrpg.threed.engine.tool.CameraProgram;
...
public class EncounterLogic {
	...
	private void turnCameraToUnit(EncounterUnitData data, boolean reset)
	{
		...
		if (J3DCore.getInstance().getClassicInputHandler().isMouseLookOn()) return;
		...
		try 
		{
			...
			float deltaY = 0;
			Vector3 finalPos = gameLogic.core.getCurrentLocation().clone();
			if (data.getCurrentLine()>=1)
			{
				finalPos.setY(gameLogic.core.getCurrentLocation().getY()+1.5f);
				deltaY = -1f;
			} else
			{
				deltaY = -0.5f;
			}
			Vector3 place = gameLogic.core.mEngine.calculatePositionVector(data.visibleForm.renderedUnit, data.visibleForm,true);
			place.setY( place.getY()+deltaY);
			
			CameraProgram p = new CameraProgram(c, new Vector3(c.getLocation()), finalPos, new Vector3(c.getDirection()), place, 0.6f);
	...
	{
		//TODO ardor3d Input!if (J3DCore.getInstance().getClassicInputHandler().getMouseLookHandler().isEnabled()) return;
		...
		Camera c = gameLogic.core.getCamera();
		Vector3 place = J3DCore.turningDirectionsUnit[gameLogic.core.gameState.getCurrentRenderPositions().viewDirection].clone();
		place.setY( place.getY()+0.2f);
		CameraProgram p = new CameraProgram(c, new Vector3(c.getLocation()), gameLogic.core.getCurrentLocation(), new Vector3(c.getDirection()), place, 0.6f);
		gameLogic.core.cameraUtil.startProgram(p);
	...
	public void handleImpactEffects(Impact impact, TurnActMemberChoice choice)
	{
		...
		if (impact.success)
		{
			gameLogic.core.uiBase.hud.mainBox.addEntry(new TextEntry("HIT!",TextEntry.RED));
		} else
		{
		...
		if (choice.targetMember!=null && choice.targetMember.encounterData!=null && choice.targetMember.encounterData.visibleForm!=null)
		{
	...
	public void finishEncounterWin(EncounterInfo encounter)
	{
		...
		gameLogic.core.uiBase.hud.mainBox.addEntry(new TextEntry("Encounters finished", TextEntry.YELLOW));
		gameLogic.endPlayerEncounters();
		gameLogic.core.getInputHandler().setForbidWindowEventHandling(false);
			...
			} else
			{
				...
				if (turnActTurnState.getCurrentEvent().type == PlannedTurnActEvent.TYPE_MEMBER_CHOICE) 
				{
					...
					if (choice.skillActForm!=null || choice.doUse)
					{
						...
						{
							gameLogic.core.uiBase.hud.mainBox.addEntry(new TextEntry(choice.member.description.getName() + " " + choice.member.memberState.getBlockingEffectText(choice.skillActForm),TextEntry.MAGENTA));
							playTurnActStep();
							return;
						}
					}
