		...
		public Impact evaluate()
		{
			...
			for (EvaluatedSkillActFormTargetData data:targetData) {
				...
				if (!sourceData.sourceChoice.isConstructive())
				{
					...
					i.messages.add(new TextEntry(data.target.description.getName() + ": Resistance Mul ("+targetRes+" ? "+resistanceRoll+") "+resString,TextEntry.DARKGRAY) );
				} else
				{
					impact = 1f;
				}
				...
				{
					i.messages.add(new TextEntry(data.target.description.getName() + ": "+data.randomTargetBodyPart.getName(), TextEntry.BLACK));
				}
				if (sourcePower>targetPower)
				{
					...
					i.messages.add(new TextEntry(data.target.description.getName() + ": Success! Impact: "+impactString+ " Def/Att: "+targetPower+"/"+sourcePower,TextEntry.RED));
					// success
					...
					if (sourceData.form!=null)
					{
						for (StateEffectInitParams params:sourceData.form.stateEffectsAndLevels)
						{
							...
							i.messages.add(new TextEntry(data.target.description.getName() + " " + e.getAdditionText(),TextEntry.MAGENTA));
							effects.add(e);
						}
					}
						...
						if (effectType == SkillActForm.EFFECTED_POINT_HEALTH)
						{
							...
							if (data.randomTargetBodyPart!=null)
							{
								...
								if (criticalHitMultiplicator>1.2f)
									i.messages.add(new TextEntry("*CRITICAL BODYPART HIT*",TextEntry.BLACK));
							}
					...
					i.messages.add(new TextEntry("Effect: "+effectText,TextEntry.BLACK));
				...
				{
					i.messages.add(new TextEntry(data.target.description.getName() + ": Failure! Def/Att: "+targetPower+"/"+sourcePower,TextEntry.BLACK));
				}
			}
			...
			i.messages.add(new TextEntry("XP Gain: "+i.actCost.experiencePoint ,TextEntry.BLACK));
			return i;
		}
