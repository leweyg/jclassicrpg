...
public class GameStateContainer {
	...
	public void switchToEncounterScenario(boolean on, String encWorldType)
	{
		...
		synchronized (mutex)
		{
			if (on)
			{
				J3DCore.getInstance().getInputHandler().switchToEncounterModeMouse(true);
	            J3DCore.getInstance().setCalculatedCameraLocation();
				...
				// TODO ardor3d J3DCore.getInstance().getKeyboardHandler().setCurrentStandingEngine(J3DCore.getInstance().eEngine);
				...
				J3DCore.getInstance().terrainHelper.switchRendering(false);
				positionPlayerToSurfaceEncounterMode();
				J3DCore.getInstance().setCalculatedCameraLocation();
				Vector3 dir = J3DCore.directions[getEncounterPositions().viewDirection];
				J3DCore.getInstance().eEngine.switchOn(true);
				JClassicRPGClassicControl.setCameraDirection(J3DCore.getInstance().getCamera(), dir.getX(), dir.getY()-0.2f, dir.getZ());
			...
			{
				J3DCore.getInstance().getInputHandler().switchToEncounterModeMouse(false);
				currentSEngine = J3DCore.getInstance().sEngine; 
				currentRenderPositions = normalPosition;
				J3DCore.getInstance().setCalculatedCameraLocation();
				Vector3 dir = J3DCore.directions[getNormalPositions().viewDirection];
				JClassicRPGClassicControl.setCameraDirection(J3DCore.getInstance().getCamera(), dir.getX(), dir.getY(), dir.getZ());
				J3DCore.getInstance().sEngine.switchOn(true);
				J3DCore.getInstance().terrainHelper.switchRendering(true);
				J3DCore.getInstance().eEngine.switchOn(false);
				if (J3DCore.getInstance().coreFullyInitialized)
				{
					J3DCore.getInstance().updateTimeRelated();
				}
				// TODO ardor3d input! J3DCore.getInstance().getKeyboardHandler().setCurrentStandingEngine(J3DCore.getInstance().sEngine);
			}
		}
	}
	...
	public void updatePerceptionRelated()
	{
		...
		Vector3 vParty = new Vector3(player.theFragment.roamingBoundary.posX, player.theFragment.roamingBoundary.posY, player.theFragment.roamingBoundary.posZ);
		...
		if (player.theFragment.perceptedEntities!=null)
		{
			for (PerceptEntityData data:player.theFragment.perceptedEntities)
			{
				...
				Vector3 point = new Vector3(i.roamingBoundary.posX,i.roamingBoundary.posY,i.roamingBoundary.posZ);//new Vector3(commonRadius[1][0],commonRadius[1][1],commonRadius[1][2]);
				eData.dist =  data.getUpdatedDist();
				eData.angle = data.distanceKnown?vParty.subtract(point,null).normalize(null).smallestAngleBetween(new Vector3(1f,0,1f))*114.6f:null;
		...
		if (list!=null)
		{
			for (Object o:list)
			{
				...
				data.angle = vParty.subtract(point).normalize().angleBetween(new Vector3(1f,0,1f))*114.6f;
	...
	public void checkAndHandleEnterLeave()
	{
		...
		if (p!=null)
		{
			...
			if (!lastPopulationName.equals(name)) {
				J3DCore.getInstance().uiBase.hud.mainBox.addEntry(new TextEntry("Entering town "+name, TextEntry.RED));
				gameLogic.core.uiBase.hud.startFloatingOSDText("Entering town "+p.town.foundationName, TextEntry.YELLOW);
		...
		if (g!=null)
		{
			...
			if (!lastGeographyName.equals(name)) {
				J3DCore.getInstance().uiBase.hud.mainBox.addEntry(new TextEntry("Entering "+name, TextEntry.RED));
				gameLogic.core.uiBase.hud.startFloatingOSDText("Entering "+name, TextEntry.WHITE);
			...
			// climate related sounds
				{
					...
					if (c.getBelt()!=null && c.getBelt().audioDescriptor!=null)
					{
						...
						if (sound!=null)
						{
							if (J3DCore.getInstance().audioServer!=null) J3DCore.getInstance().audioServer.playLoading(sound, "climate");
						}
					}
				}
			}
	...
	public boolean isPlayerLightOn()
	{
		if (player!=null)
			return player.theFragment.fragmentState.isLighting;	
		return false;
	}
