...
const target=(await (await fetch(`http://127.0.0.1:${process.env.JCRPG_CDP_PORT??9224}/json/list`)).json()).find(t=>t.type==='page'),ws=new WebSocket(target.webSocketDebuggerUrl);await new Promise(r=>ws.addEventListener('open',r,{once:true}));let id=0;const pending=new Map(),errors=[];
...
const out=process.argv[2]??'/tmp/jcrpg-interaction-evidence';fs.mkdirSync(out,{recursive:true});
const report=[];
for(const file of ['chunks/x025_z028.scene.json','chunks/x024_z028.scene.json','chunks/x006_z021.scene.json','regions/r005_005.scene.json']){
 await send('Page.navigate',{url:(process.env.JCRPG_EDITOR_ORIGIN??'http://127.0.0.1:8790')+'/editor/?file_path='+encodeURIComponent('/world/'+file)});await new Promise(r=>setTimeout(r,1000));
 ...
 const shot=await send('Page.captureScreenshot',{format:'png'});fs.writeFileSync(out+'/editor-'+file.replaceAll('/','_')+'.png',Buffer.from(shot.data,'base64'));
}
console.log(JSON.stringify({report,errors},null,2));fs.writeFileSync(out+'/editor-report.json',JSON.stringify({report,errors},null,2));ws.close();if(errors.length)process.exitCode=1;
